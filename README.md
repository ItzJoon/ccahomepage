...
공지/뉴스 작성·수정 화면에서 "이메일로 알림 보내기"를 켜서, 원하는 대상에게 이메일을
보낼 수 있습니다(등록과 동시에 자동 발송되지는 않습니다 — 실수로 전체 발송되는 사고를
막기 위해 항상 사람이 확인하고 눌러야 나갑니다).

### 동작 방식
- **대상 선택**(`EmailSendPanel.tsx`, `PostManager.tsx`의 글 수정 패널 안): 일반
  공지/뉴스는 전체 학생·교사 / 특정 학년(10·11·12 다중 선택) / 특정 학급(다중 선택) /
  직접 입력한 이메일 주소 중 고를 수 있습니다. **교과/학급 공지(teacher가 씀)는 별도
  선택 없이 항상 자동 대상**(교과는 `student_subjects` 수강 등록, 학급은
  `directory_members.homeroom`)으로만 나갑니다 — 서버가 게시글 타입을 보고 강제합니다.
- **미리보기 → 확인 → 발송**: "대상자 확인"을 누르면 서버가 실제 수신자 수·목록(처음
  20명 + "외 N명")을 계산해서 보여주고, "발송"을 눌러야 브라우저 확인창(`confirm()`)이
  한 번 더 뜬 다음에야 실제로 나갑니다. 이미 보낸 적 있는 공지는 버튼이 "재발송"으로
  바뀌고 확인창에 이전 발송 시각·인원이 함께 표시됩니다.
