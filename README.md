...
- 공지·뉴스는 작성·수정만 가능(교과/학급 공지는 작성 불가 — teacher 전용), **삭제는 불가**
  (임시저장으로 바꿔 숨기는 것은 가능)
- 공지·뉴스·Q&A 질문·게시판 글을 **일시 숨김/숨김 해제**할 수 있습니다(삭제와 달리 되돌릴 수
  있고, 숨긴 글도 관리자 화면에서는 계속 보입니다). "게시판 관리"(`/admin/board`)에서
  게시판 글도 같은 방식으로 숨김 처리·삭제할 수 있습니다(삭제는 여전히 admin 이상 전용).
...
- 조직 활동: 조직 일정·활동기록 작성/수정/삭제, 안건 상태 변경 — sub_editor와 동일한 범위지만
  editor는 이 화면 외에 위에 적힌 다른 관리 화면들도 함께 접근 가능
- 첨부파일 업로드/삭제
