...
- **이메일 발송 로그(`/admin/email-logs`) 열람 가능**(editor는 메뉴 자체가 안 보이고
  RLS도 `is_admin()`으로 editor를 제외) — 공지/뉴스 등록 시 자동 발송된 이메일의
  성공/실패 내역을 확인할 수 있습니다(17절 참고)
...
공지/뉴스가 새로 등록되면(발행 상태로) 대상 학생 전원에게 이메일을 보냅니다. 담임/교과
공지는 기존 대상 필터링 로직(교과는 `student_subjects`, 학급은 `directory_members.homeroom`)
을 그대로 재사용해서 실제 대상자에게만 보냅니다.

### 동작 방식
- `PostManager.tsx`에서 **새 글**이 `published` 상태로 저장되면, 응답을 기다리지 않고
  `/api/send-notice-email`(Route Handler, Node.js 런타임)을 백그라운드로 호출합니다.
  글 수정 시에는 다시 보내지 않습니다(등록 시 1회).
...
- 발송은 [Nodemailer](https://nodemailer.com)로 Gmail SMTP(앱 비밀번호 방식)를 통해
  ...
  속도를 제한하고, 건별로 실패하면 최대 2회까지 재시도합니다.
- 모든 발송 시도(성공/실패)는 `email_notification_logs` 테이블에 기록되고,
  `/admin/email-logs`(admin 이상 전용)에서 확인할 수 있습니다.
- 학생은 `/mypage`에서 "새 공지사항 이메일 알림 받기" 체크를 끄면 이후 발송 대상에서
  즉시 빠집니다(`profiles.email_notifications`, 기본값 켜짐).
