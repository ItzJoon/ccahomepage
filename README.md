...
└─ src/
    ...
    │   ├─ page.tsx                # 학생 홈
    │   ├─ notices/                # 공지사항 목록/상세
    │   ├─ organizations/          # 학생자치회 소개 (부서 목록/상세)
    │   ├─ members/                # 구성원 소개
    ...
    │       ├─ events/               # 일정 관리(첨부파일)
    │       ├─ organizations/        # 부서 관리("부서 목록"/"부서 구성원" 탭, 순서 변경)
    │       ├─ members/              # (구버전 URL — 부서 구성원 탭과 같은 컴포넌트)
    │       ├─ rules/                # 규정 관리(첨부파일)
    │       ├─ qna/                  # Q&A 답변 작성
    │       ├─ notify/               # 실시간 알림 발송
...
| `organizations` | 학생자치회 부서 (임원회/대의원회/사법위원회/총회 등, 관리자가 자유 추가) |
| `members` | 부서별 구성원 프로필 |
...
| `proposals` | 부서 활동 안건함. `organizations.id`로 소속 부서 연결, 상태(검토중/승인/반려/완료) |
| `proposal_votes` | 안건 찬반 투표. `(proposal_id, user_id)` 유니크로 중복 투표 방지 |
| `org_events` | 부서별 내부 일정(회의/행사/마감/일반). 학사일정 `events`와는 별개 |
| `org_records` | 부서별 활동기록(공지/활동/회의록). 게시물 `posts`와는 별개 |
...
- **읽기**: 공개 콘텐츠(부서/구성원/일정/규정/발행된 게시물)는 로그인 여부와 무관하게 누구나 열람.
...
- **로그인하려면 학교 명단(`directory_members`)에 `is_allowed=true`로 등록돼 있어야 합니다.**
  ...
  막혀 사이트를 이용할 수 없습니다(admin/superadmin만 예외). 이 차단 기능 자체는
  "외부 계정 관리"(`/admin/access-requests`)의 스위치로 켜고 끌 수 있고, 꺼두면 명단에 없는
  계정도 정상적으로 로그인·이용할 수 있습니다(기본값은 켜짐).
- 공개 콘텐츠 열람: 부서/구성원/일정/규정/발행된 공지·뉴스/커스텀 페이지 (비로그인도 열람 가능)
- 홈 화면에서 이번 달 급식표 사진 열람(비로그인도 가능) — 업로드는 admin 이상만
- 구성원 조회(`/members`, 로그인 필요): 학교 전체 학생/교사 명단을 학년·반/과목 기준으로 검색·필터,
  다른 사람의 프로필(`/members/[id]`, 획득한 뱃지 포함) 열람
- 배너·팝업 알림 확인
- Q&A: 질문 작성, 공개 질문 열람, 본인 비공개 질문 열람, **본인 질문 삭제**
- 게시판(`/board`, 기능 스위치로 꺼져 있으면 메뉴·URL 모두 접근 불가): 글/댓글(1단 대댓글까지)
  작성, **본인 글 삭제**. 글쓰기·댓글에는 도배 방지 쿨다운/시간당 횟수 제한이 걸려 있습니다.
- 닉네임 클릭 → 신고 가능(게시판 글/댓글, 프로필 공통 UI), 신고 내역은 admin 이상만 열람
- 통합 검색(`/search`)으로 공지/뉴스/Q&A/게시판/일정을 한 번에 검색 — RLS가 이미 걸러준
  것만 보임. 학생자치회 소개(`/organizations`)/생활규정(`/rules`)은 통합 검색 대상이 아니라
  각 페이지 안에 자체 검색창이 따로 있음
- 게시판 댓글·답글이 달리거나 Q&A 질문에 답변이 달리면 헤더 알림(🔔)으로 알림
- 글쓰기 중 입력 내용은 브라우저에 자동 임시저장되어(서버 전송 없음) 페이지를 벗어났다
  돌아와도 복원되고, 등록에 성공하면 지워집니다.
- 공지사항 목록·상세에는 작성자 이름도 함께 표시됩니다(공지/뉴스/게시판 모두 profiles
  select RLS를 우회하지 않고 이름만 안전하게 반환하는 `author_name` computed column 사용).
- **부서 활동(`/org-activities`, 안건함·부서 일정·활동기록)은 더 이상 student가 볼 수
  없습니다** — 헤더 메뉴에서도 빠졌고 URL로 직접 들어가도 middleware가 막습니다(아래
  "임원회 전용" 설명 참고).
- 공지사항: 일반 공지 외에 **교과 공지**(`student_subjects`에 해당 과목이 있는 경우만)와
  **학급 공지**(본인 `directory_members.homeroom`이 대상과 일치하는 경우만)도 같은 목록에
  함께 노출됩니다 — 대상이 아니면 애초에 조회되지 않습니다(RLS로 서버에서 필터링).
- 마이페이지: 닉네임·자기소개·프로필 사진 수정 (`email`/`role`은 트리거로 변경 차단),
  공지사항 이메일 알림 수신 여부 토글(기본 켜짐, 17절 참고)
...
- **접속 통계(`/admin/stats`)**: 전체 학생 접속 기록(실시간, 이름/이메일 검색·기간 필터 가능),
  연속 접속일수 상위 목록
- **사이트 잠금(`/admin/maintenance`)**: 점검 모드 켜고 끄기 — 켜져 있는 동안에도
  admin/superadmin은 사이트를 평소처럼 이용 가능(잠금의 영향을 받지 않음)
- **활동 로그(`/admin/activity-logs`)**: 관리자·에디터의 데이터 변경(추가/수정/삭제/승인 등)을
  DB 트리거로 자동 기록한 감사 로그 열람 — 사용자/행위/대상/기간으로 필터링, 변경 전후
  데이터 비교. RLS로 superadmin만 조회 가능하고 클라이언트가 직접 기록을 쓸 수 없습니다.
...
- **부서 활동(안건함/부서 일정/활동기록)이 임원회 전용으로 바뀌었습니다.** `/org-activities`는
  ...
  등록·투표는 이제 이 화면을 통해서만 가능해, 사실상 임원회 전용 기능이 되었습니다
  (다만 `proposals` 테이블의 INSERT RLS 자체는 여전히 "로그인한 본인" 기준이라 API를
...
- **부서 구성원 관리는 별도 메뉴가 아니라 `/admin/organizations`("부서 관리") 화면의
  "부서 구성원" 탭입니다.** 메인 헤더의 "구성원"(학교 전체 명단, `/members`)과 이름이
  겹쳐 헷갈리기 쉬워서 AdminNav에서 독립 메뉴로 두지 않고 이 화면 안으로 옮겼습니다
  (`OrgMembersManager.tsx`, `/admin/organizations`와 `/admin/members` 양쪽에서 재사용 —
  옛 `/admin/members` URL도 같은 컴포넌트를 그대로 보여주도록 남겨뒀습니다).
- 구성원을 추가/수정할 때 "계정 연결"로 로그인된 사용자를 이름·이메일로 검색해 연결할 수
  있습니다. 연결하면 이름이 자동으로 채워지고, 그 계정의 마이페이지 프로필 사진이 구성원
  카드에 함께 표시됩니다(소개글은 계정과 무관하게 계속 직접 입력). editor 이상만 계정
  목록을 검색할 수 있도록 `profiles_select_editor_or_above` 정책이 추가되었습니다.
- "부서 목록" 탭에서 부서를 선택하면 그 부서의 소속 구성원 목록과 "+ 구성원 추가"가
  바로 보여서, 탭을 옮기지 않고도 구성원을 추가/삭제할 수 있습니다(계정 연결 방식은
  동일). 소개글 등 세부 항목은 "부서 구성원" 탭에서 편집합니다.
...
- 여기(`/admin/notify`)의 배너/팝업은 **접속 중인 학생 화면에만** Realtime으로 즉시 반영되는
  것과 별개로, **공지사항 이메일 알림**은 접속하지 않은 학생에게도 이메일로 알릴 수 있습니다
  — `/admin/notify`의 "이메일 발송 이력" 탭에서 과거 발송 이력을 조회할 수 있고, 실제 발송은
  공지/뉴스 작성·수정 화면(`PostManager.tsx`)에서 대상을 골라 보냅니다. 자세한 내용은 17절
  참고.

## 11. 부서 활동 (안건함 / 부서 일정 / 활동기록)

> **업데이트**: 아래는 처음 만들 때 기준 설명입니다. 이후 `/org-activities`가 임원회
> 전용으로 바뀌면서 **학생 헤더 메뉴에서 빠졌고, `is_council=true`(또는 superadmin)가
> 아니면 URL로 직접 들어가도 middleware가 막습니다.** 안건 등록·투표도 사실상 임원회만
> 쓰는 기능이 됐습니다(7절 "is_council / is_judiciary" 참고). 아래 RLS 설명(`proposals_insert_own`
> 등)은 여전히 유효합니다 — 막힌 건 UI/라우팅이지 RLS가 아닙니다.

기존 `/org-activities`(원래는 헤더 메뉴 "부서 활동")에서 기존 `organizations`(임원회/대의원회/
사법위원회/총회 등)를 그대로 활용해 부서별로 아래 3가지를 제공합니다.
...
기존 DB에 반영하려면 `supabase/schema.sql` 하단의 "기능: 부서 활동" 블록을 실행하세요
(전체 재실행도 안전합니다).
...
- **구성원 조회(`/members`, 기존 "구성원" 메뉴를 교체)**: 로그인한 사용자만 열람할 수 있는
  학교 전체 인물 디렉토리입니다(개인정보 보호를 위해 비로그인 열람 불가). 기존 부서별 임원
...
헤더·푸터·홈 화면(`/`)과 관리자 화면의 헤더/사이드바 색상·테두리·폰트를 통째로 바꿀 수
...
이메일은 나가지 않습니다 — 실수로 전체 발송되는 사고를 막기 위해 항상 대상자 수를
먼저 보여주고 확인을 받습니다).

### 공지 작성 진입점이 두 곳입니다
- **`/admin/notices`** — 기존 관리자 화면. 목록 + 작성/수정 폼을 함께 보여줍니다.
- **`/notices`(학생이 보는 공개 공지사항 목록)** — teacher/editor/admin/superadmin
  계정에게만 "+ 새 공지 작성" 버튼이 보이고, 누르면 관리자 화면에 들어가지 않고도 같은
  자리에서 모달로 작성할 수 있습니다. 두 곳 모두 `PostManager.tsx` 컴포넌트 하나를
  그대로 재사용합니다(`hideList`/`autoStartNew` prop으로 "목록 없이 작성 폼만, 열자마자
  새 글 모드로" 동작하게 만든 것뿐 — 권한 분기·검증 로직은 완전히 동일합니다). teacher가
  누르면 교과/학급 공지 유형만, editor 이상이 누르면 일반 공지 유형이 보이는 기존 규칙도
  그대로 적용됩니다.

### 동작 방식
- **대상 선택**(`EmailAudienceSelector.tsx`, 작성 폼 안에 바로 펼쳐짐): 일반 공지/뉴스는
  전체 학생·교사 / 특정 학년(10·11·12 다중 선택) / 특정 학급(다중 선택) / 직접 입력한
  이메일 주소 중 고를 수 있습니다. **교과/학급 공지(teacher가 씀)는 별도 선택 없이 항상
  자동 대상**(교과는 `student_subjects` 수강 등록, 학급은 `directory_members.homeroom`)
  으로만 나갑니다 — 서버가 게시글 타입을 보고 강제합니다.
- **미리보기 → 확인 → 게시+발송이 한 번에**: "게시하기"를 누르면 (1) 서버가 실제
  수신자 수·문구를 계산해서 브라우저 확인창(`confirm()`)에 보여주고, (2) 확인해야만
  실제로 글 저장 → 이메일 발송 순서로 진행됩니다. 아직 저장 전인 새 글도 유형/대상
  조건만으로 미리보기가 가능합니다(수신자 이메일 목록 자체는 글 내용과 무관하기 때문).
  **이메일 발송이 실패해도 글 저장은 이미 끝난 뒤라 되돌리지 않습니다** — 결과 메시지로
  "공지가 게시되었습니다. 다만 이메일 발송에는 실패했습니다: …" 처럼 분리해서 보여줍니다.
  이미 보낸 적 있는 공지를 다시 저장하면서 이메일 체크를 켜면(재발송) 확인창에 이전
  발송 시각·인원이 함께 표시됩니다.
- **대상자 이메일 조회**: 전체/학년별/학급별은 `directory_members`(학교 명단, 실제
  학년·학급 정보를 갖고 있는 테이블)에서 조건에 맞는 이메일을 뽑습니다. 직접 입력 모드는
  명단 조회 없이 입력한 주소를 그대로 씁니다. 명단 기반으로 뽑은 이메일 중
  `profiles.email_notifications=false`(마이페이지에서 알림을 꺼둔 학생)는 발송 직전에
  제외됩니다 — 직접 입력한 주소에는 이 필터가 적용되지 않습니다(관리자가 의도적으로
  지정한 주소이므로).
- **권한 경계**(서버에서 강제, 클라이언트 숨김은 보조 수단일 뿐): teacher는 본인이 쓴
  교과/학급 공지만 발송 가능. 일반 공지/뉴스의 "전체 학생/교사" 발송은 admin 이상만
  가능하고, editor는 학년별/학급별/직접 입력만 쓸 수 있습니다.
- 이 API는 호출자가 실제로 공지 작성 권한이 있는 역할(teacher/editor/admin/superadmin)인지
  먼저 확인한 뒤, **서비스 롤 키**(`SUPABASE_SERVICE_ROLE_KEY`, RLS 완전 우회)로 대상자
  이메일 목록을 계산합니다 — 여러 학생의 정보를 한 번에 조회해야 해서 일반 RLS로는
  불가능하기 때문입니다.
- 발송은 [Nodemailer](https://nodemailer.com)로 Gmail SMTP(앱 비밀번호 방식)를 통해
  이뤄집니다. `pool: true` + `rateLimit: 5`(초당 5통)로 Gmail 발송 한도를 넘기지 않게
  속도를 제한하고, 건별로 실패하면 최대 2회까지 재시도합니다. 발송 전 미리보기에서
  오늘(KST) 이미 시도한 발송 건수를 더해 Gmail 일일 한도(약 500통)를 넘길 것 같으면
  경고 문구를 보여줍니다(차단은 아니고 경고만).
- **발송 이력**: 발송 1건마다 `email_notification_batches`(누가/언제/어떤 공지/어떤
  대상 조건/대상자 수/성공·실패 건수)를 남기고, 수신자별 성공/실패는
  `email_notification_logs`에 남아 각 batch에 연결됩니다. `/admin/notify`(알림 발송)
  화면의 **"이메일 발송 이력"** 탭에서 조회 — 행을 클릭하면 수신자별 상세(실패 사유
  포함)가 펼쳐집니다. admin 이상은 전체 이력을, teacher/editor는 본인이 보낸 이력만
  볼 수 있습니다(RLS `sent_by = auth.uid() or is_admin()`).
- 학생은 `/mypage`에서 "새 공지사항 이메일 알림 받기" 체크를 끄면 이후 명단 기반 발송
  대상에서 즉시 빠집니다(`profiles.email_notifications`, 기본값 켜짐).

### 환경 변수 설정 (필수)
`.env.local`(로컬)과 Vercel 프로젝트 설정(배포)에 아래 세 값을 반드시 추가해야
이메일 발송이 동작합니다. 값이 없으면 발송 API가 에러를 반환할 뿐 사이트의 다른
기능에는 영향을 주지 않습니다.

```
SUPABASE_SERVICE_ROLE_KEY=   # Supabase 대시보드 > Project Settings > API > service_role
GMAIL_USER=ccastudenthomepage@gmail.com
GMAIL_APP_PASSWORD=          # 아래 "Gmail 앱 비밀번호 발급" 참고
```

**Gmail 앱 비밀번호 발급**: 발송 계정(`ccastudenthomepage@gmail.com`)에 먼저
2단계 인증을 켜야 앱 비밀번호를 만들 수 있습니다. Google 계정 관리 > 보안 >
2단계 인증 > 앱 비밀번호에서 새로 발급받은 16자리 값을 공백 없이
`GMAIL_APP_PASSWORD`에 넣으세요(로그인 비밀번호 자체를 넣으면 동작하지 않습니다).

### 한계 / 알아둘 점
- 대상자 전원에게 **순차 발송**하고 응답이 올 때까지 서버 함수가 계속 실행되는 방식이라,
  Vercel의 서버리스 실행 시간 제한(`maxDuration = 300`으로 설정해뒀지만 플랜별 상한이
  다름) 안에서 끝나야 합니다. 학교 규모가 지금보다 훨씬 커져서 한 번에 수천 통을 보내야
  한다면, 이 방식 대신 별도 큐(예: 메시지 큐 + 워커)로 바꾸는 걸 고려해야 합니다.
- Gmail 일반 계정은 **하루 약 500통** 한도가 있습니다. 이 한도 자체를 코드가 막아주지는
  않으므로, 재학생 수가 그 이상이면 Google Workspace 계정(하루 2,000통)으로 바꾸거나
  발송 대상을 나눠야 합니다.
