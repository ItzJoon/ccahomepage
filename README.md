...
- **조직 활동(`/org-activities`, 안건함·조직 일정·활동기록)은 더 이상 student가 볼 수
...
### sub_editor
- `/admin/org-activities/*` **한 영역만** 접근 가능(안건함·조직 일정·활동기록 — 그 외 모든
...
- 안건 상태 변경(검토중/승인/반려/완료), 조직 일정·활동기록 작성/수정/삭제 가능
...
- 콘텐츠 생성·수정: 조직·구성원·일정·규정·공지/뉴스·커스텀 페이지·메뉴·메인화면 블록
- 조직·구성원은 **삭제까지 가능**
...
- 조직 활동: is_council이 있어야 조직 일정·활동기록 작성/수정/삭제, 안건 상태 변경 가능
...
- **이메일 발송 이력(`/admin/notify`의 "이메일 발송 이력" 탭) 전체 열람 가능** — teacher/
  ...
  (17절 참고)
- 나머지 관리 화면(조직/구성원/일정/규정/Q&A 답변/메인화면 편집/페이지 빌더 등)은 editor와 동일
...
- **조직 활동(안건함/조직 일정/활동기록)은 `is_council` 여부와 무관하게 항상 접근
...
- "학생회 임원회"·"사법위원회"는 아직 role/조직 체계에 정식으로 없어서(이슈 #21, 회의로
...
- **조직 활동(안건함/조직 일정/활동기록)이 임원회 전용으로 바뀌었습니다.** `/org-activities`는
...
- `is_council=true`인 계정은 `/org-activities`에 "임원회 캘린더" 탭이 추가로 보입니다 —
  모든 부서의 조직 일정(`org_events`)을 조직 구분 없이 모아서 보여줍니다.
...
기존 `/org-activities`(원래는 헤더 메뉴 "조직 활동")에서 기존 `organizations`(임원회/대의원회/
