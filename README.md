...
- 로그인한 학생이 홈 화면이나 마이페이지에 접속하면, 버튼을 누르지 않아도 그날 첫 방문 시 자동으로
  체크인되고 화면 우측 하단에 "오늘 접속 체크 완료! 연속 N일째" 토스트가 잠깐 떴다 사라집니다
  (`useAutoCheckIn`). 연속 접속일수가 뱃지 조건(기본 3/7/30/100일)에 도달하면 그 순간 자동으로
...
- `/admin/notify`에서 발송할 때 **표시 시간**을 고를 수 있습니다: "계속 표시"(기본값, 학생이 직접
  닫기 전까지 유지 — 기존 동작과 동일) 또는 10분/30분/1시간/3시간/24시간. 시간이 지정된 알림은
  그 시간이 지나면 배너가 자동으로 사라집니다(`notifications.duration_minutes`).
- 발송 이력에서 "삭제" 버튼으로 지난 알림 기록을 지울 수 있습니다.
