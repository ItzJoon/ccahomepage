...
import type { JudiciaryEvent } from "@/lib/types";

const CATEGORY_LABEL: Record<JudiciaryEvent["category"], string> = {
  meeting: "회의",
  event: "행사",
  deadline: "마감",
  general: "일반",
...
const empty = {
  ...
  description: "",
  location: "",
  category: "meeting" as JudiciaryEvent["category"],
  start_at: "",
  end_at: "",
};

/** 사법위원회 전용 일정 — OrgEventsManager를 복사했지만 judiciary_events(별개 테이블)를 쓴다. */
export default function JudiciaryEventsManager() {
  const supabase = createClient();
  const { t } = useHomeTheme();
  const { rows: events, reload } = useRealtimeList<JudiciaryEvent>("judiciary_events", { orderBy: { column: "start_at" } });
  ...
  const startEdit = (e: JudiciaryEvent) => {
    const next = {
      title: e.title,
  ...
  const save = async () => {
    ...
    if (!form.title.trim() || !form.start_at || !form.end_at) {
      setError("일정명, 시작·종료 시간을 확인해 주세요.");
      return;
    }
    ...
    if (editing === "new") {
      ...
      await supabase.from("judiciary_events").insert({ ...payload, created_by: user?.id });
    } else if (editing) {
      await supabase.from("judiciary_events").update(payload).eq("id", editing);
    }
    setEditing(null);
    reload();
  };
  ...
  const remove = async (id: string) => {
    ...
    await supabase.from("judiciary_events").delete().eq("id", id);
    ...
    <div className={`grid grid-cols-1 gap-[18px] items-start ${editing ? "lg:grid-cols-[1fr_360px]" : ""}`}>
      <div className="min-w-0">
        <div className="flex justify-between items-end mb-4">
          <h2 className="text-[22px]">사법위원회 · 일정</h2>
        ...
        {mode === "month" ? (
          ...
          <AdminTable>
            ...
            <tbody>
              ...
              {events.length === 0 && <tr><td colSpan={4} className="text-muted text-center py-8 text-sm">등록된 일정이 없습니다.</td></tr>}
            </tbody>
          </AdminTable>
        )}
      </div>
      {editing && (
        <div className={`${t.adminEditPanel} flex flex-col gap-1.5 sticky top-20`}>
          ...
          <select className={t.adminInput} value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value as JudiciaryEvent["category"] })}>
