...
-- 104. 관리자 계정을 회원/체크인 관련 통계에서 전부 제외
-- ------------------------------------------------------------
-- "전체 가입자 수", 뱃지 보유 인원 수, 하루 방문 횟수, 연속 접속 순위 등 사람 수를 세는
-- 통계에 admin/superadmin(관리자) 계정이 섞여 있었다. claim_easter_egg_badge()의 정원
-- 계산도 그동안 superadmin만 빼고 admin은 포함하고 있었는데, 이번에 admin도 함께 뺀다
-- (다른 통계들과 기준을 통일).
create or replace function claim_easter_egg_badge()
returns json as $$
declare
...
begin
  ...
  select count(*) into v_holder_count
    from user_badges ub join profiles p on p.id = ub.user_id
    where ub.badge_id = v_badge_id and p.role not in ('admin', 'superadmin')
      and not exists (select 1 from members m where m.user_id = p.id);
  if v_holder_count >= 10 then
    update badges set is_active = false where id = v_badge_id;
    raise exception '이미 정원(10명)이 마감된 뱃지입니다';
...
-- ------------------------------------------------------------
-- 110. 스트릭 프리즈 자동 재충전 (연속 7일마다 1개, 최대 3개)
-- ------------------------------------------------------------
-- 지금까지는 가입 시 freeze_credits가 1개 지급된 뒤로 다시 채워주는 경로가 전혀 없어서,
-- 한 번이라도 프리즈를 쓴 계정은 그 뒤로 영영 프리즈 선택 모달을 볼 수 없었다(사용자
-- 보고: "연속 접속 일수 프리즈를 쓸지 선택하는 창이 안 뜨는 거 같은데" — 실제로는 모달
-- 로직 자체는 정상 작동했고, 재충전 수단이 없어서 그런 것으로 확인됨). 연속 접속을
-- 계속 이어가는 것에 대한 보상 겸 재충전 수단으로, 스트릭이 7의 배수를 새로 달성할
-- 때마다 프리즈 1개를 자동 지급한다(무한 축적 방지를 위해 최대 3개로 캡).
create or replace function check_in_attendance(p_user_id uuid, p_use_freeze boolean default false)
returns json as $$
declare
  v_today date := (timezone('Asia/Seoul', now()))::date;
...
-- ------------------------------------------------------------
-- 119. 공지사항에 사진 첨부 기능 추가
-- ------------------------------------------------------------
-- 지금까지 공지(posts)는 파일 다운로드용 attachments 테이블과 뉴스 전용 동영상
-- 임베드(video_url 등)만 있고, 게시판(board_posts.image_url)처럼 본문에 사진 한 장을
